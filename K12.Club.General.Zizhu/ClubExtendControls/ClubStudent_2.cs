...
namespace K12.Club.General.Zizhu
{
    //社團的學生增減
    //必須有另一個背景模式進行處理
    [FISCA.Permission.FeatureCode("K12.Club.Universal.ClubStudent.cs", "第二阶段参与学生")]
    public partial class ClubStudent_2 : DetailContentBase
    {
        ...
        void BGW_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            //本社團的學生參與記錄
            scMAG = new ScJoinMag(this.PrimaryKey, 2);
        ...
        private void BindData()
        {
            ...
            lbCourseCount.Text = "有效社员/所有状态社员：" + scMAG.StudentDic.Keys.Count + "/" + scMAG.SCJoinStudent_LIst.Count.ToString();

            btnInserStudent.Text = "由待处理加入社员(" + K12.Presentation.NLDPanels.Student.TempSource.Count.ToString() + ")";
        ...
        private void AddListViewInTemp(List<string> IsSaft)
        {
            if (IsSaft.Count != 0)
            {
                ...
                //排除已存在本課程的學生
                cso.CheckTempStudentInCourse(IsSaft, scMAG.SCJoin_Dic, _CLUBRecord, 2);
        ...
        private void ClearStudent()
        {
            ...
            //Log
            StringBuilder sb_Log = new StringBuilder();
            sb_Log.AppendLine(string.Format("已移除「{0}」名社团学生：(学年度「{1}」学期「{2}」社团「{3}」)", list.Count.ToString(), _CLUBRecord.SchoolYear.ToString(), _CLUBRecord.Semester.ToString(), _CLUBRecord.ClubName));
