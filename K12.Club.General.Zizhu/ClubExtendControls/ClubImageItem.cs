...
namespace K12.Club.General.Zizhu
{
    //因為照片屬於比較大的檔案
    //因此儲存時多提供一個背景模式
     [FISCA.Permission.FeatureCode("K12.Club.Universal.ClubImageItem.cs", "拓展性课程照片")]
    public partial class ClubImageItem : DetailContentBase
    {
        ...
        void BGW_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                this.Loading = false;
                FISCA.Presentation.Controls.MsgBox.Show("取得[照片]发生错误!!\n" + e.Error.Message);
        ...
        protected override void OnSaveButtonClick(EventArgs e)
        {
            ...
            {
                FISCA.Presentation.Controls.MsgBox.Show("资料不同步\n储存失败");
                return;
            }
        ...
        void bgwPhoto_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (!e.Cancelled)
            {
                ...
                {
                    FISCA.Presentation.Controls.MsgBox.Show("资料储存失败!!\n" + e.Error.Message);
                    return;
                }
            }
            else
            {
                FISCA.Presentation.Controls.MsgBox.Show("储存作业已取消!!");
                return;
            }
        }
        ...
        private void buttonItem1_Click(object sender, EventArgs e)
        {
            ...
            if (od.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    {
                        FISCA.Presentation.Controls.MsgBox.Show("照片大小不可超过640(W)*480(H)\n您所选的图档" + orgBmp.Width + "(W)*" + orgBmp.Height + "(H)");
                        return;
                    }
                    fs.Close();
        ...
        {
            if (FISCA.Presentation.Controls.MsgBox.Show("您要清除此照片吗？", "", MessageBoxButtons.YesNo) == DialogResult.No) return;
