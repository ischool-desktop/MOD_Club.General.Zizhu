...
namespace K12.Club.General.Zizhu
{
    public partial class OpenClubJoinDateTime : FISCA.Presentation.Controls.BaseForm
    {
        ...
        {
            lblSemester.Text = string.Format("{0}学年度　第{1}学期", School.DefaultSchoolYear, School.DefaultSemester);
            ...
            else if (!string.IsNullOrEmpty(startTime) && !string.IsNullOrEmpty(endTime))
            {
                ...
                if (objStart.HasValue && objEnd.HasValue)
                {
                    if (objStart.Value >= objEnd.Value)
                        row.ErrorText = "截止时间必须在开始时间之后。";
                }
            }
            else
                row.ErrorText = "请输入正确的时间限制资料(必需同时有资料或同时没有资料)。";
        }
